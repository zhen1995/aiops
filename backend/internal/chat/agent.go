...
const (
	// MaxAgentIterations Agent 最大迭代次数，防止死循环
	MaxAgentIterations = 5
)

// AgentCallbacks 供上层（如 SSE 控制器）观察 Agent 执行过程
...
// NewAgent 创建 Agent
func NewAgent(cm model.ChatModel, registry *tools.Registry) *Agent {
	return &Agent{cm: cm, registry: registry}
}

// Run 执行 Agent 循环，返回最终答案消息（含 ResponseMeta）
func (a *Agent) Run(ctx context.Context, messages []*schema.Message, cb *AgentCallbacks) (*schema.Message, error) {
	...
	// 判断当前问题是否明显需要查询数据源
	requireData := requiresDataSource(lastUserContent(messages))
	...
	for i := 0; i < MaxAgentIterations; i++ {
		...
		if len(resp.ToolCalls) == 0 {
			...
			if requireData && len(invokedQueryTools) == 0 {
				if !reminderSent && i < MaxAgentIterations-1 {
					// 先追加一条强提醒，让模型重新决策
	...
	return nil, fmt.Errorf("Agent 超过最大迭代次数 %d", MaxAgentIterations)
}

func (a *Agent) executeToolCalls(ctx context.Context, calls []schema.ToolCall, toolMap map[string]tool.InvokableTool, cb *AgentCallbacks) []string {
	type result struct {
		idx     int
		content string
	}
