...
import { ALLOW_DELETE_OPERATION, ALLOW_DDL_OPERATION, ALLOW_INSERT_OPERATION, ALLOW_UPDATE_OPERATION, SCHEMA_DELETE_PERMISSIONS, SCHEMA_DDL_PERMISSIONS, SCHEMA_INSERT_PERMISSIONS, SCHEMA_UPDATE_PERMISSIONS, isMultiDbMode, mcpConfig as config, MCP_VERSION as version, IS_REMOTE_MCP, REMOTE_SECRET_KEY, PORT, ENABLE_PII_REDACTION, PII_EXTRA_COLUMNS, PII_EXTRA_COLUMN_PATTERNS, } from "./src/config/index.js";
    ...
    server.setRequestHandler(ReadResourceRequestSchema, async (request) => {
        try {
            ...
            const filtered = ENABLE_PII_REDACTION
                ? results.filter((col) => !isPIIColumn(col.column_name, piiColumnList, PII_EXTRA_COLUMN_PATTERNS))
                : results;
            if (ENABLE_PII_REDACTION && filtered.length !== results.length) {
                log("info", `[redact] hid ${results.length - filtered.length} PII column(s) from schema for table "${tableName}"`);
            }
            return {
                contents: [
                    {
